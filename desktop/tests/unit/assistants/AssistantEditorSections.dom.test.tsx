...
// The app's names for pi's thinking levels (the `mu.levels` keys), in English.
const THINKING_LEVEL_NAMES: Record<string, string> = { low: 'Low', medium: 'Medium', high: 'High' };
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (_key: string, options?: { defaultValue?: string; count?: number }) => {
      ...
      if (_key.startsWith('mu.levels.')) return THINKING_LEVEL_NAMES[_key.slice('mu.levels.'.length)] ?? _key;
...
describe('AssistantEditorSections', () => {
  ...
  it('keeps builtin and disabled MCP servers in the default MCP summary', () => {
    ...
    const defaultsCard = screen.getByTestId('assistant-card-defaults');
    expect(within(defaultsCard).getByText('User MCP, Disabled MCP, and Builtin MCP')).toBeInTheDocument();
  });

  it('joins the default MCP summary in the app language', () => {
    mockLanguage = 'zh-CN';
    mockResolvedLanguage = 'zh-CN';
    renderWithProviders(
      <AssistantEditorSections
        editor={createEditor({
          ...
          defaults: {
            mcps: {
              ...
              selectedIds: ['mcp-user', 'mcp-builtin'],
              setSelectedIds: vi.fn(),
            },
          },
        })}
