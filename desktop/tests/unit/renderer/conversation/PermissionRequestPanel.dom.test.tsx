...
describe('PermissionRequestPanel', () => {
  ...
  it.each(['execute', 'edit', 'read', 'fetch', 'tool'] as const)(
    'names the %s operation kind in the reader language without a header icon',
    (operationKind) => {
      renderPanel({ operationKind });
      const card = screen.getByTestId('message-permission-card');
      expect(within(card).getByText(`messages.permissionKind.${operationKind}`)).toBeInTheDocument();
      expect(within(card).queryByText(operationKind)).not.toBeInTheDocument();
