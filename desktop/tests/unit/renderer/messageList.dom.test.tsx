...
vi.mock('@/renderer/pages/conversation/runtime/useConversationRuntimeView', () => ({
  useConversationRuntimeView: () => ({ isProcessing: mockIsProcessing, hydrated: mockHydrated }),
}));
...
vi.mock('@/renderer/pages/conversation/Messages/components/MessageThinking', () => ({
  // Exposes what the list decided for the row: live or a kept record, open or closed.
  default: ({
    message,
    active,
    expanded,
    ...
    <div
      data-testid={`thinking-${active ? 'active' : 'record'}-${message.id}`}
      data-expanded={String(Boolean(expanded))}
    >
      <button onClick={() => onExpandedChange?.(message.id, !expanded)}>{`toggle ${message.id}`}</button>
    </div>
  ),
  ThoughtHistory: ({ messages }: { messages: IMessageThinking[] }) => (
    <div data-testid='thinking-history'>{messages.map((message) => message.content.content).join(',')}</div>
  ),
}));
...
// Lets a test drive the list the way live stream merges do, one update at a time.
...
describe('MessageList', () => {
  ...
  it('shows one completed-thinking disclosure per turn and only the latest active indicator', () => {
    const messages = [
      createThinkingMessage('thought-1', 'before tool', 'done', 1),
      createAcpToolCall({ id: 'tool-1' }),
      createThinkingMessage('thought-2', 'after tool', 'done', 3),
      createAcpToolCall({ id: 'tool-2' }),
      createThinkingMessage('thought-3', 'older active', 'thinking', 5),
      createThinkingMessage('thought-4', 'latest active', 'thinking', 6),
      {
        id: 'user-1',
        msg_id: 'user-1',
        conversation_id: 'conversation-1',
        ...
        position: 'right',
        created_at: 7,
        content: { content: 'next turn' },
      },
      createThinkingMessage('thought-5', 'next turn active', 'thinking', 8),
    ] as TMessage[];
    // A stored `thinking` status alone no longer lights the live row: the turn has to be running.
    ...
    // Historical thinking is one disclosure for the turn, even when tools split
    // the stream. The earlier active state is retained there rather than shown
    // as a second live indicator.
    expect(screen.getAllByTestId('thinking-history')).toHaveLength(1);
    expect(screen.getByTestId('thinking-history')).toHaveTextContent(
      'before tool,after tool,older active,latest active'
    );
    expect(screen.queryByTestId('thinking-active-thought-4')).not.toBeInTheDocument();
    expect(screen.queryByTestId('thinking-record-thought-4')).not.toBeInTheDocument();
    expect(screen.getByTestId('thinking-active-thought-5')).toBeInTheDocument();
    expect(screen.getByTestId('thinking-active-thought-5')).toHaveAttribute('data-expanded', 'false');

    const content = screen.getByTestId('message-list-content');
    expect(content.querySelectorAll('.tool_summary')).toHaveLength(2);
  });

  describe('thinking state flow', () => {
    ...
    const liveRows = () => screen.queryAllByTestId(/^thinking-active-/);

    it('does not show a leftover thought as live when the conversation is idle', () => {
      // Reopened history or a switched-to conversation: the status was never completed, nothing runs.
      renderList([createUserMessage('user-1', 1), createThinkingMessage('thought-1', 'left behind', 'thinking', 2)]);

      expect(liveRows()).toHaveLength(0);
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('left behind');
    });

    it('does not revive a thought left behind by an earlier turn while a later turn runs', () => {
      mockIsProcessing = true;
      renderList([
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'unfinished turn', 'thinking', 2),
        createUserMessage('user-2', 3),
        createAcpToolCall({ id: 'tool-1' }),
      ]);

      expect(liveRows()).toHaveLength(0);
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('unfinished turn');
    });

    it('keeps a cancelled thought out of the next turn until that turn has its own thought', () => {
      const cancelled = [
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'cancelled mid-thought', 'thinking', 2),
      ];
      mockIsProcessing = true;
      const { rerender } = renderList(cancelled);
      expect(screen.getByTestId('thinking-active-thought-1')).toBeInTheDocument();

      // Stop: the runtime goes idle, the message keeps its `thinking` status forever.
      mockIsProcessing = false;
      rerender(
        <>
          <MessageList />
      ...
      expect(liveRows()).toHaveLength(0);
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('cancelled mid-thought');

      // Next send: processing starts before the new turn's rows arrive. The old thought is still
      // the last row, and must not light up again.
      mockIsProcessing = true;
      rerender(
        <>
          <MessageList />
      ...
      replaceMessages([
        ...cancelled,
        createUserMessage('user-2', 3),
        createThinkingMessage('thought-2', 'new turn', 'thinking', 4),
      ]);
      expect(liveRows()).toHaveLength(1);
      expect(screen.getByTestId('thinking-active-thought-2')).toBeInTheDocument();
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('cancelled mid-thought');
    });

    it('lights a running conversation on entry, before the runtime view has hydrated as idle', () => {
      // Re-entering a conversation mid-turn: the first frames are unhydrated, then processing. The
      // live thought must not be written off as stale during that window.
      mockHydrated = false;
      const { rerender } = renderList([
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'still streaming', 'thinking', 2),
      ]);
      expect(liveRows()).toHaveLength(0);
    ...
    it('moves a closed live thought into the turn history when a tool call ends it', () => {
      mockIsProcessing = true;
      renderList([createUserMessage('user-1', 1), createThinkingMessage('thought-1', 'plan', 'thinking', 2)]);
      expect(screen.getByTestId('thinking-active-thought-1')).toHaveAttribute('data-expanded', 'false');
      expect(screen.queryByTestId('thinking-history')).not.toBeInTheDocument();

      replaceMessages([
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'plan', 'done', 2),
        createAcpToolCall({ id: 'tool-1' }),
      ]);

      // No "thought complete" row is left between the tools: the thought is in the history only.
      expect(screen.queryByTestId(/^thinking-(active|record)-/)).not.toBeInTheDocument();
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('plan');
      expect(screen.getByTestId('tool-summary')).toBeInTheDocument();
    });

    it('never counts a thought as live once a row follows it, even if its completion frame was lost', () => {
      mockIsProcessing = true;
      renderList([
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'completion frame lost', 'thinking', 2),
        createAcpToolCall({ id: 'tool-1' }),
      ]);

      expect(liveRows()).toHaveLength(0);
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('completion frame lost');
    });

    it('keeps an opened thought open and in place through completion, until the reader closes it', () => {
      mockIsProcessing = true;
      renderList([createUserMessage('user-1', 1), createThinkingMessage('thought-1', 'plan', 'thinking', 2)]);
      fireEvent.click(screen.getByRole('button', { name: 'toggle thought-1' }));
      const row = screen.getByTestId('thinking-active-thought-1');
      expect(row).toHaveAttribute('data-expanded', 'true');
      const rowElement = row.closest('.message-item');
      ...
      // Same row, still open, no longer live; not duplicated into the history.
      const record = screen.getByTestId('thinking-record-thought-1');
      expect(record).toHaveAttribute('data-expanded', 'true');
      expect(record.closest('.message-item')).toBe(rowElement);
      expect(screen.queryByTestId('thinking-history')).not.toBeInTheDocument();

      fireEvent.click(screen.getByRole('button', { name: 'toggle thought-1' }));
      expect(screen.queryByTestId('thinking-record-thought-1')).not.toBeInTheDocument();
      expect(screen.getByTestId('thinking-history')).toHaveTextContent('plan');
    });

    it('keeps an opened thought open when the turn is cancelled under it', () => {
      mockIsProcessing = true;
      const { rerender } = renderList([
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'interrupted', 'thinking', 2),
      ]);
      fireEvent.click(screen.getByRole('button', { name: 'toggle thought-1' }));
      ...
      expect(liveRows()).toHaveLength(0);
      expect(screen.getByTestId('thinking-record-thought-1')).toHaveAttribute('data-expanded', 'true');
    });

    it('keeps the turn history mounted while later thoughts move into it', () => {
      mockIsProcessing = true;
      const first = [
        createUserMessage('user-1', 1),
        createThinkingMessage('thought-1', 'first', 'done', 2),
        createAcpToolCall({ id: 'tool-1' }),
        createThinkingMessage('thought-2', 'second', 'thinking', 4),
      ];
      renderList(first);
      const history = screen.getByTestId('thinking-history');
      expect(history).toHaveTextContent('first');
      expect(screen.getByTestId('thinking-active-thought-2')).toBeInTheDocument();

      replaceMessages([
        ...first.slice(0, 3),
        createThinkingMessage('thought-2', 'second', 'done', 4),
        createAcpToolCall({ id: 'tool-2' }),
        createThinkingMessage('thought-3', 'third', 'thinking', 6),
      ]);

      // The same DOM node: a remount here would silently close a history the reader had opened.
      expect(screen.getByTestId('thinking-history')).toBe(history);
