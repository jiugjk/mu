...
vi.mock('@renderer/pages/conversation/utils/conversationCreateError', () => ({
  ...
  normalizeConversationRuntimeWorkspaceErrorCode: () => undefined,
}));
...
describe('TaskDetailPage', () => {
  ...
  it('shows a translated headline when run-now fails for an unknown reason', async () => {
    runNowInvokeMock.mockRejectedValue(new Error('scheduler offline'));
    ...
    await waitFor(() => expect(Message.error).toHaveBeenCalledWith('cron.runNowFailed'));
  });
  ...
  it('names the saved reasoning effort as a thinking level, not the raw id', async () => {
    ...
    expect(await screen.findByText('acp.config.reasoning_effort')).toBeInTheDocument();
    expect(screen.getByText('mu.levels.xhigh')).toBeInTheDocument();
  ...
  it('shows the latest execution error when hovering the failed status', async () => {
    ...
    expect(screen.queryByText('cron.detail.lastRunFailed')).not.toBeInTheDocument();
    expect(screen.queryByText('ACP init failed: config file is invalid')).not.toBeInTheDocument();

    fireEvent.mouseEnter(status);

    expect(await screen.findByText('cron.detail.lastRunFailed')).toBeInTheDocument();
