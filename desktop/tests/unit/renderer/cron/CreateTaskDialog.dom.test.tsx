...
vi.mock('@renderer/pages/conversation/utils/conversationCreateError', () => ({
  ...
  normalizeConversationCreateErrorCode: () => undefined,
}));
...
describe('CreateTaskDialog', () => {
  ...
  it('does not save advanced mode without a cron expression', async () => {
    ...
    // The field shows the problem; the validator's English "form validate error" must not become a toast.
    expect(Message.error).not.toHaveBeenCalled();
  });

  it('shows a translated headline with the backend detail when saving fails', async () => {
    const user = userEvent.setup();
    vi.mocked(ipcBridge.cron.updateJob.invoke).mockRejectedValue(new Error('database is locked'));
    ...
    await waitFor(() => expect(Message.error).toHaveBeenCalledWith('cron.page.updateFailed'));
  });
