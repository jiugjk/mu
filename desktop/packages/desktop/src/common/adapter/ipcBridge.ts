 ...
 */
const nativeSyncMainLanguage = bridge.buildProvider<void, { language: string }>('system-settings:sync-main-language');
const syncMainLanguage = {
  provider: nativeSyncMainLanguage.provider,
  invoke: async (params: { language: string }): Promise<void> => {
    if (isElectronRenderer()) {
      await nativeSyncMainLanguage.invoke(params);
    }
  },
};
const patchLanguageSetting = httpPatch<void, { language: string }>('/api/settings', (p) => ({ language: p.language }));
...
export const systemSettings = {
  ...
  /**
   * Saves the language to the backend and makes the main process follow it (`syncMainLanguage`). The main process
   * registers its handler on `syncMainLanguage`; this `provider` is a no-op like every HTTP-backed entry.
   */
  changeLanguage: {
    provider: patchLanguageSetting.provider,
    invoke: async (params: { language: string }): Promise<void> => {
      // Not awaited: the main process answers after it rebuilt its menus, and the save must not wait for that.
      void syncMainLanguage.invoke(params).catch(() => {});
      await patchLanguageSetting.invoke(params);
    },
  },
  syncMainLanguage,
...
interface IBridgeResponse<D = {}> {
  success: boolean;
  data?: D;
  /** Raw failure message: for logs, or as a secondary detail under a translated headline. */
  msg?: string;
  /** Stable failure code the renderer maps to a translated text, when the provider has one. */
  code?: string;
}
