...
const TaskDetailPage: React.FC = () => {
  ...
  const handleRunNow = useCallback(async () => {
    ...
    try {
      ...
      // An unavailable project folder has a sentence of its own; anything else is the backend's raw text, shown as
      // the detail under a translated headline.
      Message.error(
        normalizeConversationRuntimeWorkspaceErrorCode(err)
          ? getConversationRuntimeWorkspaceErrorMessage(err, t)
          : t('cron.runNowFailed', { error: parseError(err) })
      );
    } finally {
      runningNowRef.current = false;
      setRunningNow(false);
    }
  }, [job, t, navigate, i18n.language, isNewConversationMode]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      Message.error(t('cron.deleteFailed', { error: parseError(err) }));
    }
  }, [job, navigate, t]);
          ...
          <div className='flex flex-wrap items-center gap-10px md:gap-12px'>
              ...
              <Tooltip
                ...
                content={
                  <div className='max-w-360px whitespace-pre-wrap break-words'>
                    <div className='mb-4px text-12px font-medium'>{t('cron.detail.lastRunFailed')}</div>
                    <div className='text-12px leading-18px opacity-80' dir='ltr'>
                      {latestExecutionError}
                    </div>
                  </div>
                }
            ...
            {job.state.next_run_at_ms && (
              <span className='text-14px text-t-secondary'>
                {t('cron.nextRunAt', { time: formatNextRun(job.state.next_run_at_ms, i18n.language) })}
              </span>
            )}
          </div>
            ...
            {job.metadata.agent_config?.config_options &&
              Object.keys(job.metadata.agent_config.config_options).length > 0 && (
                <section className='flex flex-col gap-10px'>
                  ...
                  <span className='break-words text-14px leading-22px text-t-primary'>
                    {formatNameList(
                      Object.values(job.metadata.agent_config.config_options).map((value) =>
                        thinkingLevelLabel(t, value)
                      ),
                      i18n.language
                    )}
                  </span>
