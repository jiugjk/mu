...
import { parseError, resolveLocaleKey } from '@/common/utils';
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const handleSubmit = async () => {
    // A rejected validation needs no toast: the form shows each field's problem inline, and the validator's own
    // message is English developer text.
    ...
    try {
      ...
      // Known workspace and assistant problems have sentences of their own; anything else is the backend's raw
      // text, shown as the detail under a translated headline.
      Message.error(
        normalizeConversationCreateErrorCode(err)
          ? getConversationCreateErrorMessage(err, t)
          : t(isEditMode ? 'cron.page.updateFailed' : 'cron.page.createFailed', { error: parseError(err) })
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <Form form={form} layout='vertical'>
          ...
          {frequency === 'custom' && (
            <div className='mb-16px rounded-12px border border-solid border-[var(--color-border-2)] p-14px'>
              ...
              {customSchedule.mode !== 'advanced' && (
                <div className='mt-12px flex flex-wrap items-baseline gap-x-8px rounded-8px bg-[var(--color-fill-1)] px-12px py-10px text-12px text-t-secondary'>
                  <span>{t('cron.page.custom.preview')}</span>
                  <span className='font-mono text-t-primary' dir='ltr'>
                    {scheduleInfo.expr}
                  </span>
                </div>
              )}
