...
const GuidPage: React.FC = () => {
  ...
  // The picker's first value is the catalog's memory of the last session, not a choice: mu starts a new
  // conversation in the mode the settings set. So a mode is sent only once the person picked one here, or the
  // assistant has its own; until then the picker shows the mode mu will start in.
  const muStartMode = useMuStartMode();
  const [modePicked, setModePicked] = useState(false);
  useEffect(() => {
    setModePicked(false);
  }, [selectedAssistantId, location.key]);
  const startMode =
    !modePicked && muStartMode && agentSelection.currentAgentModeOptions.some((mode) => mode.value === muStartMode)
      ? muStartMode
      : agentSelection.selectedMode;
    ...
    const applyAssistantDefaults = async () => {
      ...
      if (resolvedDefaults.permissionMode) {
        ...
        if (availableModeIds.size === 0 || availableModeIds.has(resolvedDefaults.permissionMode)) {
          ...
          // The assistant's own mode is a choice, made where the assistant was set up.
          setModePicked(true);
        } else {
