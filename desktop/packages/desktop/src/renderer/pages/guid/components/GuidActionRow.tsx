  ...
  const sheetEntries = useMemo<MobileActionSheetEntry[]>(() => {
    ...
    if (thoughtLevelOption && thoughtLevelOption.options.length > 0 && onThoughtLevelSelect) {
      ...
      entries.push({
        ...
        meta: getCurrentThoughtLevelLabel(t, thoughtLevelOption),
        submenu: {
          title: t('agent.thoughtLevel.label'),
          options: thoughtLevelOption.options.map((o) => ({
            key: o.value,
            label: thoughtLevelOptionLabel(t, o),
      ...
      entries.push({
        ...
        submenu: {
          ...
          options: mcpServers.map((server) => ({
            key: server.id,
            label: server.name,
            description: server.tools?.length ? t('mcp.toolCount', { count: server.tools.length }) : undefined,
        ...
        <Menu.SubMenu
          ...
          title={
            <div className='flex items-center gap-8px'>
              ...
              <span>
                {t('common.labelWithSelectedCount', {
                  label: t('settings.capabilitiesTab.skills'),
                  selected: activeSkillCount,
              ...
              >
                ...
                >
                  <span className='text-13px'>
                    {server.tools?.length
                      ? t('mcp.serverWithToolCount', { name: server.name, count: server.tools.length })
                      : server.name}
                  </span>
                </Checkbox>
              </Menu.Item>
