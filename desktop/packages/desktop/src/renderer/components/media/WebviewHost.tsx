...
export interface WebviewHostProps {
  ...
  /**
   * Takes over the report of this webview's webContents id. When set, the id goes to the caller INSTEAD of the
   * single-target CDP bridge: that bridge attaches and detaches a debugger of its own as tabs change, which would
   * cut off whoever else is driving the page.
   */
  onWebContentsReady?: (webContentsId: number, webview: Electron.WebviewTag) => void;
  /**
   * Leave the page's own behaviour alone: no interception of link clicks, window.open and form submissions. The
   * interception re-issues a form's action as a plain navigation, without the form's data, which is fine for a
  ...
  useEffect(() => {
    ...
    const handleDomReady = () => {
      ...
      try {
        ...
        if (typeof webContentsId === 'number' && onWebContentsReadyRef.current) {
          onWebContentsReadyRef.current(webContentsId, webviewEl);
        } else if (typeof webContentsId === 'number') {
        ...
        <div className='aion-url-viewer-toolbar flex items-center gap-6px h-40px px-10px bg-bg-2 border-b border-border-1 flex-shrink-0'>
          ...
          {isStarOffice && (
            <div className='flex items-center gap-6px ms-2px'>
              <button onClick={handleZoomReset} className='toolbar-btn' title={t('preview.zoomActualSize')}>
                {formatNumber(1, i18n.language, { style: 'percent' })}
              </button>
              <button onClick={handleZoomFit} className='toolbar-btn' title={t('preview.zoomFitTooltip')}>
                {t('preview.zoomFit')}
              </button>
              <span className='toolbar-chip'>
                {formatNumber(zoomFactor, i18n.language, { style: 'percent', maximumFractionDigits: 0 })}
              </span>
            </div>
          )}
