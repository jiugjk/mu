...
import { getGateway } from "./outbound/outbound-service.ts";
import { getPackageVersion, setPackageVersion } from "./utils/pkg-version.ts";

/** mu 适配：原版记录 OpenClaw 框架版本；移植后框架即 mu，版本同 QQ 通道 */
...
export function buildUserAgent(suffix?: string): string {
	const base = `QQBotPlugin/${getPackageVersion()} (Node/${process.versions.node}; ${os.platform()}; mu/${getPackageVersion()})`;
