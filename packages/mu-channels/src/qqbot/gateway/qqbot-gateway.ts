...
import { createWebhookServerAdapter } from "../adapter/webhook.ts";
import { buildUserAgent } from "../bot-instance.ts";
import { getCachedMsgId } from "../features/msgid-cache.ts";
...
import { handleInteraction, handleMessage } from "./event-handlers.ts";
...
export interface SendOptions {
	/** 被动回复 msgId；null = 明确发主动消息（被动回复限额已用完，不从缓存补 msgId） */
	msgId?: string | null;
	text?: string;
}
...
function withTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
	...
	return Promise.race([
		promise,
		new Promise<T>((_, reject) => setTimeout(() => reject(new Error(`出站超时: ${label} (${ms}ms)`)), ms)),
	]);
}
...
export class QQBotGateway {
	...
	private readonly runtime: QQBotRuntime | undefined;
	readonly log: PluginLogger;
	private readonly textTimeout: number;
	private readonly mediaTimeout: number;

	/** 完整模式：接收 + 发送（含中间件、session 持久化、ref-index） */
	constructor(account: ResolvedQQBotAccount, runtime: QQBotRuntime, log?: PluginLogger);
	/** Send-only 模式：仅发送（无中间件、无 session、无 ref-index） */
	constructor(account: ResolvedQQBotAccount);
	constructor(account: ResolvedQQBotAccount, runtime?: QQBotRuntime, log?: PluginLogger) {
		this.textTimeout = resolveMs("MU_QQBOT_OUTBOUND_TIMEOUT_MS", TEXT_TIMEOUT_MS);
		this.mediaTimeout = resolveMs("MU_QQBOT_OUTBOUND_MEDIA_TIMEOUT_MS", MEDIA_TIMEOUT_MS);
		...
		this.bot = new QQBot({
			...
			webhook: isWebhook
				? { path: account.config.webhook?.path, server: createWebhookServerAdapter({ account, log: this.log }) }
				: undefined,
	...
	async sendText(target: ReplyTarget, text: string, opts?: SendOptions): Promise<MessageResponse> {
		return withTimeout(this.bot.sendText(attachMsgId(target, opts), text), this.textTimeout, "sendText");
	}
	...
	async sendTextWithKeyboard(
		...
		return withTimeout(
			this.bot.sendTextWithKeyboard(attachMsgId(target, opts), text, keyboard as never),
			this.textTimeout,
	...
	async sendVideo(target: ReplyTarget, source: string, opts?: SendOptions): Promise<MessageResponse> {
		const resolvedTarget = attachMsgId(target, opts);
		const sourceOpts = resolveMediaSource(source);
		const result = await withTimeout(
			this.bot.sendMedia({
			...
			session.complete = async (): Promise<any> => {
				...
				if (result?.ext_info?.ref_idx) {
					getPersistedRefIndexStore(accountId).set(result.ext_info.ref_idx, {
						...
						senderName,
						timestamp:
							typeof result.timestamp === "number" ? new Date(result.timestamp).toISOString() : result.timestamp,
						isBot: true,
...
function attachMsgId(target: ReplyTarget, opts?: SendOptions): ReplyTarget {
	...
	if (opts?.msgId === null) {
		const { msgId: _drop, ...proactive } = target;
		return proactive;
	}
	// 无显式 msgId 时尝试从缓存获取
