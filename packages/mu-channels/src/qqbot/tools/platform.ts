...
import { getRequestAccountId } from "../request-context.ts";

// ========== Schema ==========
// mu 适配：原版为 JSON Schema 对象（OpenClaw registerTool）；pi 的工具参数用 TypeBox 描述，字段与说明不变。

const PlatformApiSchema = Type.Object({
	method: Type.Union(
 ...
 */
export function createPlatformTool(defaultAccountId?: string): ToolDefinition {
	// mu 适配：原版在插件注册时对 OpenClaw 全局注册（无账户时跳过）；移植后作为 QQ 会话的自定义工具创建
	const tool: ToolDefinition<typeof PlatformApiSchema> = {
		...
		async execute(_toolCallId, params) {
			...
			const accountId = getRequestAccountId() ?? defaultAccountId;
			if (!accountId) {
