	...
	const walk = (dir: string) => {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				try {
					files.push({ filePath: full, size: fs.statSync(full).size });
				} catch {
					/* skip */
				}
			}
...
export function botClearStorage(account: ResolvedQQBotAccount): SlashCommand {
	// mu 适配：下载按会话隔离在 media/<账户>/<c2c|group>/<openid>/downloads，清理本账户全部会话的媒体文件
	const targetDir = getQQBotMediaDir(safeSegment(account.accountId));
