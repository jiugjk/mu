...
import { getBotForAccount } from "../bot-instance.ts";
import { resolveQQBotAccount } from "../config.ts";
...
/**
 * 已知用户存储
 * 使用简单的 JSON 文件存储，保存在 ~/.mu/qqbot/data 目录下
 */
import { getQQBotDataDir } from "../utils/platform.ts";

// mu 适配：原版在模块加载时解析目录；移植后在使用时解析（mu home 由启动流程决定）
function storageDir(): string {
	return getQQBotDataDir("data");
}

function knownUsersFile(): string {
	return path.join(storageDir(), "known-users.json");
}
...
function saveKnownUsers(users: Map<string, KnownUser>): void {
	try {
		...
		fs.writeFileSync(knownUsersFile(), JSON.stringify(data, null, 2), "utf-8");
...
/**
 * 主动发送消息（带配置解析）
 * 注意：与 outbound.ts 中的 sendProactiveMessage 不同，这个函数接受 MuConfig 并自动解析账户
 *
 * @param options - 发送选项
 ...
 * ```
 */
export async function sendProactive(options: ProactiveSendOptions, cfg: MuConfig): Promise<ProactiveSendResult> {
 ...
 */
export function getUserGroups(accountId: string, _openid: string): string[] {
	const users = loadKnownUsers();
	const groups: string[] = [];
