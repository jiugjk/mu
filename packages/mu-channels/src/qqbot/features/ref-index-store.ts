...
export class PersistedRefIndexStore implements RefIndexStore {
	...
	private async appendToDisk(key: string, entry: RefEntry): Promise<void> {
		...
		try {
			...
			log.error(`append failed: ${err instanceof Error ? err.message : String(err)}`);
		}
	}
	...
	private async compact(): Promise<void> {
		...
		try {
			...
			this.diskLineCount = lines.length;
			log.info(`compacted to ${lines.length} entries`);
		} catch (err) {
			log.error(`compact failed: ${err instanceof Error ? err.message : String(err)}`);
			// 清理可能残留的 tmp 文件
			try {
	...
	private compactSync(): void {
		...
		try {
			...
			log.error(`compactSync failed: ${err instanceof Error ? err.message : String(err)}`);
		}
	}
