...
/** mu 适配：QQ 通道随 mu 发布，检查 mu-agent 的版本（原版检查 @tencent-connect/openclaw-qqbot） */
const PKG_NAME = "mu-agent";
...
async function fetchDistTags(log?: PluginLogger): Promise<Record<string, string>> {
	// mu 适配：尊重 mu 的离线开关（PI_OFFLINE，测试与离线环境）
	if (process.env.PI_OFFLINE) throw new Error("offline (PI_OFFLINE)");
...
function buildUpdateInfo(tags: Record<string, string>): UpdateInfo {
	const currentIsPrerelease = getPackageVersion().includes("-");
	...
	const hasUpdate =
		...
		compareTarget !== getPackageVersion() &&
		compareVersions(compareTarget, getPackageVersion()) > 0;
...
export function triggerUpdateCheck(log: PluginLogger): void {
	getUpdateInfo(log)
		.then((info) => {
			if (info.hasUpdate) {
				log.info(`[update-checker] new version available: ${info.latest} (current: ${getPackageVersion()})`);
			}
		})
		.catch(() => {});
}
...
function buildRegistries(pkgName: string): string[] {
	...
	return [`https://registry.npmjs.org/${encoded}`, `https://registry.npmmirror.com/${encoded}`];
}
