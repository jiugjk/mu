...
/**
 * mu.json（~/.mu/agent/mu.json）中本通道读取的部分。
 * 原版为 OpenClawConfig；移植后 QQ Bot 配置位于 `channels.qqbot`，结构与 OpenClaw 相同。
 */
...
/** mu 权限模式（见 kyrn-judge permissions）：full=完全访问 | jev=Jev 审批 | ask=最小权限 */
export type MuPermissionMode = "full" | "jev" | "ask";

/**
 * 入站限流档位（mu 移植新增，偏离原行为：原版 rateLimiter() 未配置任何档位，等于不限流）。
...
export interface SessionPoolConfig {
	/** 闲置多少分钟后关闭会话（默认 30；0 = 不回收） */
	idleMinutes?: number;
	/** 同时打开的会话上限（默认 32），超出后先关闭最久未用的会话 */
	maxSessions?: number;
}
...
export interface WebhookTransportConfig {
	...
	path?: string;
	/** 监听端口（mu 移植新增：原版复用 OpenClaw 网关的 HTTP 服务；默认 8787） */
	port?: number;
	/** 监听地址（mu 移植新增，默认 0.0.0.0） */
	host?: string;
}
...
export interface QQBotAccountConfig {
	...
	/** QQ 会话工作目录根（默认 ~/.mu/qqbot/workspace）；每个私聊/群各占一个子目录 */
	workspace?: string;
	/** QQ 会话使用的模型 "provider/model-id"（默认 mu 默认模型） */
	model?: string;
	/** QQ 会话的 mu 权限模式（默认 jev）；/bot-approve 会修改它 */
	permissions?: MuPermissionMode;
	/** 群配置面板回包里的 claw_type（默认 "mu"，可改回 "openclaw"） */
	clawType?: string;
	/** 群配置面板回包里的 @ 文本匹配（原版取自 OpenClaw 的 messages.groupChat.mentionPatterns） */
	mentionPatterns?: string[];
	/** 入站限流；false 关闭 */
	rateLimit?: RateLimitConfig | false;
	/** 会话池 */
	sessions?: SessionPoolConfig;
	/** 按钮/文字审批等待多少秒后按默认值（拒绝）处理（默认 600） */
