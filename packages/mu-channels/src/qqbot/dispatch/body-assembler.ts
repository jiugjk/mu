...
function formatMergedLine(
	...
	opts: {
		...
		isLast: boolean;
		wasMentioned: boolean;
		formatEnvelope: ((p: Record<string, unknown>) => string) | null;
	},
): string {
	const m = ctx.message;
	...
	return opts.isGroup ? `${formatSenderLabel(m.senderName, m.senderId)}: ${content}` : content;
}
...
function buildDynamicCtx(
	...
	const asrTexts = unique(transcripts.map((t) => (t.source === "asr" ? t.text : t.asrReferText)).filter(isNonEmpty));
...
function buildMsgElementsContext(msg: QQBotInboundMessage): string[] {
	...
	for (const el of elements) {
		...
		const author = (el as Record<string, unknown>).author as { username?: string } | undefined;
		const sender = author?.username ?? "未知";

		lines.push(`=== 消息 ${index} ===`, `[消息内容] ${content}`, `[发送者] ${sender}`);
	}
...
function renderWebBody(getRuntime: () => any, bodyContent: string, msg: QQBotInboundMessage, isGroup: boolean): string {
	try {
