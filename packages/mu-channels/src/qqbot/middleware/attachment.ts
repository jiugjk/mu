 ...
 *
 * mu 适配：下载目录按会话隔离（~/.mu/qqbot/media/<账户>/<c2c|group>/<openid>/downloads），
 * 原版统一下载到 ~/.openclaw/media/qqbot/downloads。
 */
import * as path from "node:path";
import type { MiddlewareContext } from "@tencent-connect/qqbot-nodejs";
import { convertSilkToWav, isVoiceAttachment } from "@tencent-connect/qqbot-nodejs/protocol";
import { downloadRemoteMedia } from "../adapter/media.ts";
...
export function attachmentProcessor(opts: AttachmentMiddlewareOptions) {
	return async (ctx: MiddlewareContext, next: () => Promise<void>) => {
		...
		if (attachments?.length) {
			...
			const peerId = (isGroup ? msg.groupOpenid : msg.senderId) ?? "unknown";
...
async function processAttachments(
	...
	const tasks = attachments.map(async (att) => {
		...
		if (isImage && url) {
			const localPath = await downloadMediaFile(url, att.filename, downloadDir, log);
...
async function processVoiceAttachment(
	...
	try {
		...
		if (!localPath) {
			...
			if (silkUrl) {
				const silkPath = await downloadMediaFile(silkUrl, att.filename, downloadDir, log);
				if (silkPath) {
	...
	return {
		...
		uploadDirectFormats: normalizeFormats(
			(policy?.uploadDirectFormats as string[]) ??
				(qqbot?.voiceDirectUploadFormats as string[]) ?? [".wav", ".mp3", ".silk"],
		),
