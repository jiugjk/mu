 ...
 */
export function dynamicAccessControl(params: { accountId: string; getRuntime: () => unknown }): Middleware {
	...
	return async (ctx, next) => {
		...
		const mode: string = isGroup ? ((p?.groupMode as string) ?? "open") : ((p?.c2cMode as string) ?? "allowlist");
		...
		ctx.log?.info?.(`[access] blocked ${isGroup ? "group" : "c2c"} from ${id}: not in ${listLabel}`);
...
async function checkPairingMode(
	ctx: any,
	next: () => Promise<void>,
	opts: { accountId: string; getRuntime: () => unknown; senderId: string },
): Promise<void> {
	...
	try {
		...
		// mu 修正：待批准的配对请求已满（每账户 3 个，与 OpenClaw 一致）时 code 为空，原版会发出空配对码
		if (!challenge.code) {
			ctx.log?.info?.(`[access] pairing queue full, ${opts.senderId} not issued a code`);
			await ctx.bot
				.sendText(ctx.replyTarget, "⚠️ 当前待批准的配对请求过多，请稍后再试或联系机器人管理员。")
				.catch(() => {
					/* ignore */
				});
			ctx.stop("access:pairing_queue_full");
			return;
		}
