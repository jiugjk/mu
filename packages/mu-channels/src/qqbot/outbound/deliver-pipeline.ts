...
import { getQQBotHome } from "../utils/platform.ts";
import type { PluginLogger } from "../utils/plugin-logger.ts";
import type { DeliverDebouncer } from "./debounce.ts";
import { isPathInAllowedRoots } from "./local-file-router.ts";
import { sendMedia } from "./media-send.ts";
import type { MediaKind, SendResult } from "./outbound-service.ts";
...
export interface DeliverContext {
	...
	textToSpeech?: (params: {
		...
		channel?: string;
		accountId?: string;
	}) => Promise<{ audioPath: string | null; error?: string }>;
...
function resolveAttachmentPaths(payload: DeliverPayload): string[] {
	...
	return attachments.map((a) => a.url ?? a.path).filter((p): p is string => typeof p === "string" && p.length > 0);
}
...
export async function deliverReply(
	...
	ctx.log?.debug(`[text] textLen=${text.length} mediaCount=${mediaUrls.length}${formatLogTextPreview(text)}`);
...
const TTS_ALLOWED_ROOTS = [getQQBotHome(), os.tmpdir(), "/tmp"];
