...
import type { QQBotGateway } from "../gateway/qqbot-gateway.ts";
...
export class StreamingController {
	...
	private readonly deps: StreamingControllerDeps;
	...
	private async sendUpdate(text: string): Promise<void> {
		...
		try {
			...
			this.deps.log?.error(
				`update failed (len=${text.length}): ${err instanceof Error ? err.message : String(err)}`,
			);
			this.session = null;
	...
	private async completeSession(reason?: string): Promise<void> {
		...
		this.deps.log?.info(
			`completing stream (sent=${this.sentChunkCount} chars=${this.lastAcceptedFull.length} reason=${reason ?? "done"})`,
		);
		try {
			await this.session.complete();
		} catch (err) {
...
export function shouldUseStreaming(account: ResolvedQQBotAccount, targetScope: "c2c" | "group" | "channel"): boolean {
