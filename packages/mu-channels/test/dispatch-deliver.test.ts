 ...
 *
 * 移植自 openclaw-qqbot tests/dispatch-deliver.test.ts：用例原样保留，test() 注册为 vitest 用例。
 */
...
import {
	...
	streamOwnsText,
} from "../src/qqbot/dispatch/dispatch-deliver.ts";
import type { DeliverContext, DeliverPayload } from "../src/qqbot/outbound/deliver-pipeline.ts";
import { formatLogTextPreview } from "../src/qqbot/utils/log-text-preview.ts";
...
test("stream owns text only after it has started and has not fallen back", () => {
...
test("kind=block first-round commentary is sent immediately", async () => {
	const { state, replies, media, logLines } = createState();
	await deliverDispatchPayload({ text: "Let me check that for you." }, { kind: "block" }, state);
	assert.deepEqual(
	...
	assert.ok(
		logLines.some((line) => line.includes("deliver kind=block") && line.includes("Let me check that for you.")),
	);
});

test("kind=block duplicate text is skipped", async () => {
...
test("kind=block forwards new media then sends leftover text", async () => {
	const { state, replies, media } = createState();
	await deliverDispatchPayload({ text: "see image", mediaUrl: "https://cdn/a.png" }, { kind: "block" }, state);
...
test("kind=tool forwards media only and never sends tool text", async () => {
	const { state, replies, media } = createState();
	await deliverDispatchPayload({ text: "exec running", mediaUrl: "https://cdn/tool.png" }, { kind: "tool" }, state);
	assert.deepEqual(media, ["https://cdn/tool.png"]);
	assert.deepEqual(replies, []);
});

test("kind=final is skipped when the same block text was already sent", async () => {
	const { state, replies } = createState();
...
test("kind=final still sends a later different answer", async () => {
	const { state, replies } = createState();
...
test("kind=block voice payload goes through deliverReply intact", async () => {
	const { state, replies, media } = createState();
	await deliverDispatchPayload({ text: "hello", audioAsVoice: true }, { kind: "block" }, state);
	assert.equal(replies.length, 1);
...
test("kind=block text is skipped once the stream owns the lane", async () => {
...
test("kind=final finalizes the stream and skips static text", async () => {
	const stream = createStreamOwner({ hasStarted: true });
	const { state, replies } = createState({ streamingController: stream });
...
test("stream fallback to static sends the final payload", async () => {
	const stream = createStreamOwner({ hasStarted: true });
...
test("kind=final after a block still forwards only new media", async () => {
	const { state, replies, media } = createState();
	await deliverDispatchPayload({ text: "caption", mediaUrl: "https://cdn/a.png" }, { kind: "block" }, state);
...
test("formatLogTextPreview shows delivered text and clips long payloads", () => {
...
test("filterDeliveredMedia drops already forwarded urls", () => {
...
test("deliverDispatchPayloadSafe swallows deliver errors", async () => {
...
test("media forward failure is logged and does not throw", async () => {
	const { state, logLines } = createState();
	state.ctx.sendMedia = async () => {
		throw new Error("upload failed");
	};
	await deliverDispatchPayload({ mediaUrl: "https://cdn/a.png" }, { kind: "tool" }, state);
