 ...
 * 防止后续重构破坏协议兼容性。
 *
 * 移植自 openclaw-qqbot tests/body-assembler.test.ts：用例原样保留，test() 注册为 vitest 用例。
 */
import assert from "node:assert";
import { it } from "vitest";
import { assembleBody } from "../src/qqbot/dispatch/body-assembler.ts";
import type { ProcessedAttachments } from "../src/qqbot/middleware/attachment.ts";
import type { VoiceTranscript } from "../src/qqbot/utils/voice-text.ts";

// ── 测试基础设施（移植：原文件自带计数器与 process.exit，改为 vitest 注册） ──
...
const fakeAccountFields = {
	accountId: "acc-1",
	systemPrompt: undefined,
	config: {},
};
const fakeAccount = fakeAccountFields as never;
...
const dynCtxCases: DynCtxCase[] = [
	{
		...
		expectedContains: ["- Voice: /tmp/a.wav, https://x/a.silk", "- ASR: 哈喽 world"],
	},
	{
		...
		expectedContains: ["- Voice: https://x/b.silk", "- ASR: 语音里的话"],
	},
	{
		...
		expectedContains: ["- Voice: /tmp/a.wav, https://x/a.silk, https://x/b.silk"],
...
test("群被@且有 history：前置 [Chat messages since...] 块", () => {
	...
	const { agentBody } = assembleBody(ctx, msg, fakeAccount);
	// 移植说明：原测试期望 "[Chat messages since your last reply — CONTEXT ONLY]" / "[CURRENT MESSAGE — reply to this]"，
	// 但原版 body-assembler.ts 的常量已是 "[Chat history begins]" / "[Chat history ends]" / "[Current message]"——
	// 该用例在原仓库里即已过期。这里按源码实际协议断言。
	assert.ok(agentBody.startsWith("[Chat history begins]\n"), `agentBody=\n${agentBody}`);
...
test("systemPrompt：account.systemPrompt trim 后取值；空字符串变 undefined", () => {
	...
	const msg = makeMsg({ sanitizedContent: "hi" });
	const sp1 = assembleBody(ctx, msg, { ...fakeAccountFields, systemPrompt: "  你是助手  " } as never).systemPrompt;
	assert.strictEqual(sp1, "你是助手");
	const sp2 = assembleBody(ctx, msg, { ...fakeAccountFields, systemPrompt: "   " } as never).systemPrompt;
